...
mean(diabetes_v$Glucose[diabetes_v$newAge=="46-60"])b
...
ggplot(data = diabetes)+
  ...
  theme + labs(title = "Graph 15. Regression of Glucose on log(BMI)", x = "log(BMI)", y = "Glucose")
```
...
######
Glucose ~ Outcome + Insulin + BloodPressure + Age + newInsu + SkinThickness + newSkin
```
...
#####
Glucose ~ BloodPressure + SkinThickness + Insulin + Age + Outcome + newInsu + newSkin
```
...
#options(scipen = 999)
Glucose_modelhi<-lm(Glucose~BloodPressure + logage +  logbmi + poly(DiabetesPedigreeFunction,2)+
    newInsu + newSkin,data=train.df2)
sum_hi <- summary(Glucose_modelhi)


# show metrics
sum_hi  # adjust r^2 0.237, rmse 25.846
sqrt(mean(sum_hi$residuals^2))
```
